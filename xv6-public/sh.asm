...
fork1(void)
{
  ...
  if(buf[0] == 0) // EOF
    ...
    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      ...
      dd:	e8 9e 0b 00 00       	call   c80 <strlen>
      if(chdir(buf+3) < 0)
...
execcmd(void)
{
  ...
  memset(cmd, 0, sizeof(*cmd));
     355:	6a 00                	push   $0x0
     357:	50                   	push   %eax
     358:	e8 53 09 00 00       	call   cb0 <memset>
  cmd->type = EXEC;
...
pipecmd(struct cmd *left, struct cmd *right)
{
  ...
  memset(cmd, 0, sizeof(*cmd));
     3d5:	6a 00                	push   $0x0
     3d7:	50                   	push   %eax
     3d8:	e8 d3 08 00 00       	call   cb0 <memset>
  cmd->type = PIPE;
  cmd->left = left;
...
listcmd(struct cmd *left, struct cmd *right)
{
  ...
  memset(cmd, 0, sizeof(*cmd));
     415:	6a 00                	push   $0x0
     417:	50                   	push   %eax
     418:	e8 93 08 00 00       	call   cb0 <memset>
  cmd->type = LIST;
  cmd->left = left;
...
backcmd(struct cmd *subcmd)
{
  ...
  memset(cmd, 0, sizeof(*cmd));
     455:	6a 00                	push   $0x0
     457:	50                   	push   %eax
     458:	e8 53 08 00 00       	call   cb0 <memset>
  cmd->type = BACK;
  cmd->cmd = subcmd;
     ...
     479:	8b 45 08             	mov    0x8(%ebp),%eax
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     4a3:	e8 28 08 00 00       	call   cd0 <strchr>
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     4f3:	e8 d8 07 00 00       	call   cd0 <strchr>
  ...
  *ps = s;
  ...
  if(eq)
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     534:	e8 97 07 00 00       	call   cd0 <strchr>
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     556:	e8 75 07 00 00       	call   cd0 <strchr>
     ...
     bf1:	31 c0                	xor    %eax,%eax
{
     ...
     bf9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
     bfc:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  while((*s++ = *t++) != 0)
     c00:	0f b6 14 03          	movzbl (%ebx,%eax,1),%edx
     c04:	88 14 01             	mov    %dl,(%ecx,%eax,1)
     c07:	83 c0 01             	add    $0x1,%eax
     c0a:	84 d2                	test   %dl,%dl
     c0c:	75 f2                	jne    c00 <strcpy+0x10>
    ;
  return os;
}
     c0e:	8b 5d fc             	mov    -0x4(%ebp),%ebx
     c11:	89 c8                	mov    %ecx,%eax
     c13:	c9                   	leave  
     c14:	c3                   	ret    
     c15:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
...
strcmp(const char *p, const char *q)
{
  ...
  while(*p && *p == *q)
     ...
     c2f:	75 17                	jne    c48 <strcmp+0x28>
     c31:	eb 3a                	jmp    c6d <strcmp+0x4d>
     c33:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
     c37:	90                   	nop
     c38:	0f b6 42 01          	movzbl 0x1(%edx),%eax
    p++, q++;
     c3c:	83 c2 01             	add    $0x1,%edx
     c3f:	8d 59 01             	lea    0x1(%ecx),%ebx
  while(*p && *p == *q)
     c42:	84 c0                	test   %al,%al
     c44:	74 1a                	je     c60 <strcmp+0x40>
    p++, q++;
     c46:	89 d9                	mov    %ebx,%ecx
  while(*p && *p == *q)
     c48:	0f b6 19             	movzbl (%ecx),%ebx
     c4b:	38 c3                	cmp    %al,%bl
     c4d:	74 e9                	je     c38 <strcmp+0x18>
  return (uchar)*p - (uchar)*q;
     c4f:	29 d8                	sub    %ebx,%eax
}
  ...
  return (uchar)*p - (uchar)*q;
     ...
     c72:	eb db                	jmp    c4f <strcmp+0x2f>
     c74:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
...
strchr(const char *s, char c)
{
  ...
  for(; *s; s++)
     ...
     cef:	84 d2                	test   %dl,%dl
     cf1:	74 0d                	je     d00 <strchr+0x30>
    if(*s == c)
     cf3:	38 d1                	cmp    %dl,%cl
     ...
     d19:	31 db                	xor    %ebx,%ebx
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     d25:	57                   	push   %edi
     d26:	6a 00                	push   $0x0
     d28:	e8 2e 01 00 00       	call   e5b <read>
    if(cc < 1)
    ...
    if(c == '\n' || c == '\r')
     ...
     d41:	74 1d                	je     d60 <gets+0x50>
     d43:	3c 0d                	cmp    $0xd,%al
     d45:	74 19                	je     d60 <gets+0x50>
  for(i=0; i+1 < max; ){
     d47:	89 de                	mov    %ebx,%esi
  ...
  buf[i] = '\0';
     ...
     d54:	c6 04 30 00          	movb   $0x0,(%eax,%esi,1)
  return buf;
}
  ...
  buf[i] = '\0';
     ...
     d63:	89 de                	mov    %ebx,%esi
     d65:	c6 04 30 00          	movb   $0x0,(%eax,%esi,1)
}
     d69:	8d 65 f4             	lea    -0xc(%ebp),%esp
     d6c:	5b                   	pop    %ebx
     d6d:	5e                   	pop    %esi
     d6e:	5f                   	pop    %edi
     d6f:	5d                   	pop    %ebp
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     d88:	6a 00                	push   $0x0
     d8a:	ff 75 08             	push   0x8(%ebp)
     d8d:	e8 f1 00 00 00       	call   e83 <open>
  if(fd < 0)
     d92:	83 c4 10             	add    $0x10,%esp
     d95:	85 c0                	test   %eax,%eax
     d97:	78 27                	js     dc0 <stat+0x40>
    return -1;
  r = fstat(fd, st);
     d99:	83 ec 08             	sub    $0x8,%esp
     d9c:	ff 75 0c             	push   0xc(%ebp)
     d9f:	89 c3                	mov    %eax,%ebx
     da1:	50                   	push   %eax
     da2:	e8 f4 00 00 00       	call   e9b <fstat>
  close(fd);
     da7:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
     daa:	89 c6                	mov    %eax,%esi
  close(fd);
     dac:	e8 ba 00 00 00       	call   e6b <close>
  return r;
     db1:	83 c4 10             	add    $0x10,%esp
}
     db4:	8d 65 f8             	lea    -0x8(%ebp),%esp
...
atoi(const char *s)
{
  ...
  n = 0;
  while('0' <= *s && *s <= '9')
     dd7:	0f be 02             	movsbl (%edx),%eax
     dda:	8d 48 d0             	lea    -0x30(%eax),%ecx
     ddd:	80 f9 09             	cmp    $0x9,%cl
  n = 0;
     de0:	b9 00 00 00 00       	mov    $0x0,%ecx
  while('0' <= *s && *s <= '9')
     de5:	77 1e                	ja     e05 <atoi+0x35>
     de7:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
     dee:	66 90                	xchg   %ax,%ax
    n = n*10 + *s++ - '0';
     df0:	83 c2 01             	add    $0x1,%edx
     df3:	8d 0c 89             	lea    (%ecx,%ecx,4),%ecx
     df6:	8d 4c 48 d0          	lea    -0x30(%eax,%ecx,2),%ecx
  while('0' <= *s && *s <= '9')
     dfa:	0f be 02             	movsbl (%edx),%eax
     dfd:	8d 58 d0             	lea    -0x30(%eax),%ebx
     e00:	80 fb 09             	cmp    $0x9,%bl
     e03:	76 eb                	jbe    df0 <atoi+0x20>
  return n;
}
     e05:	8b 5d fc             	mov    -0x4(%ebp),%ebx
     e08:	89 c8                	mov    %ecx,%eax
     e0a:	c9                   	leave  
     e0b:	c3                   	ret    
     ...
     f0c:	89 d1                	mov    %edx,%ecx
{
  ...
  write(fd, &c, 1);
     ...
     f7e:	56                   	push   %esi
     f7f:	57                   	push   %edi
     f80:	e8 de fe ff ff       	call   e63 <write>
  while(--i >= 0)
     ...
     fb9:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  write(fd, &c, 1);
     ...
     ff5:	57                   	push   %edi
     ff6:	56                   	push   %esi
     ff7:	e8 67 fe ff ff       	call   e63 <write>
  ...
  write(fd, &c, 1);
    ...
    104e:	e8 10 fe ff ff       	call   e63 <write>
        putc(fd, c);
  ...
  write(fd, &c, 1);
    ...
    105f:	57                   	push   %edi
    1060:	56                   	push   %esi
    1061:	e8 fd fd ff ff       	call   e63 <write>
        putc(fd, c);
  ...
  write(fd, &c, 1);
    ...
    10cb:	57                   	push   %edi
    10cc:	56                   	push   %esi
    10cd:	e8 91 fd ff ff       	call   e63 <write>
        while(*s != 0){
  ...
  write(fd, &c, 1);
    110f:	e8 4f fd ff ff       	call   e63 <write>
        ap++;
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    1243:	e8 83 fc ff ff       	call   ecb <sbrk>
  if(p == (char*)-1)
